...
5. Inspect affected links/anchors, IDs, examples and commands against actual sources. Run doc builders, linters, executable validators or examples only with explicit scoped authorization for the current task; reuse that authorization when already provided. A documentation edit or an example being safe/useful does not authorize execution. Never run destructive setup/migration/publishing commands merely to validate documentation. Report source inspection separately from executed checks and mark unexecuted checks as not run, with remaining uncertainties.
